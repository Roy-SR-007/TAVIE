    ...
    **kwargs : dict
    ...
    while True:
        ...
        if cfunc is not None:
            elbo_full = (
                -(anew / 2) * np.log(b_xi / 2.0)
                + gammaln(anew / 2.0)
                + alpha * np.sum(cfunc(xi, **kwargs))
                + 0.5 * logdetV_xi
                - 0.5 * logdetV0
                - gammaln(a0 / 2.0)
                + (a0 / 2.0) * np.log(b0 / 2.0)
            )
            elbo.append(elbo_full)
        
        m_hist.append(m_xi.copy())
        V_hist.append(V_xi.copy())
        a_hist.append(float(anew))
        b_hist.append(float(b_xi))
...
def cfunc_student(xi: np.ndarray,
    ...
    res3 = gammaln((nu+1.0)/2.0) - gammaln(nu/2.0) - 0.5 * np.log(nu * np.pi)
